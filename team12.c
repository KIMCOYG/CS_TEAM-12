...
void readMem(unsigned int, unsigned int);
...
void readMem(unsigned int start, unsigned int end) { //m
	...
	for (unsigned int i = start; i < end; i += 4) {
		if (progMem[i] != 0) {
			printf("PC :    %X,  progMem     :    %X \n", 0x400000 + i, (progMem[i] << 24) | (progMem[i + 1] << 16) | (progMem[i + 2] << 8) | progMem[i + 3]);
		}
	}
	for (unsigned int i = start; i < end; i += 4) {
		for (unsigned int j = i; j < i + 4; j++) {
			if (dataMem[j] != 0) {
				printf("data     :    %x \n", (dataMem[i] << 24) | (dataMem[i + 1] << 16) | (dataMem[i + 2] << 8) | dataMem[i + 3]);
				break;
			}
		}
		if (stackMem[i] != 0) {
			printf("data     :    %x \n", (stackMem[i] << 24) | (stackMem[i + 1] << 16) | (stackMem[i + 2] << 8) | dataMem[i + 3]);
			break;
		}
	}
}
